<template>
  ...
  <div class="pl-12 pr-12 pt-6 flex justify-between">
    <div class="bg-zinc-900" v-if="activeToggle === true" style="width: 80%">
      <h1 class="p-2 pl-6 font-bold bg-zinc-800 text-white text-left text-3xl">BOOT-TEST-PANEL</h1>
    </div>
    <div class="bg-zinc-900" v-else style="width: 100%">
      <h1 class="p-2 pl-6 font-bold bg-zinc-800 uppercase text-white text-left text-3xl">BOOT-TEST-PANEL</h1>
    </div>
...
<!-- Results Header -->
  ...
  <!-- RFID Modal -->
  <template>
    <Dialog v-model:visible="rfidModalVisible" modal header="RFID" :style="{ width: '35%' }" :draggable="false" >
      <RfidWarningModal />
    </Dialog>
  </template>
  ...
  <!-- LABEL Modal -->
  <template>
    <Dialog v-model:visible="labelModalVisible" modal header="LABEL" :style="{ width: '35%' }" :draggable="false" position="right">
      <LabelComponent />
    </Dialog>
  </template>
...
import LabelComponent from '../components/BootTest/LabelComponent.vue';
...
onMounted(() => {
  ...
  // store.macAddress = "123456789098765434"
  // store.rfidState = true;
  // store.uuid = "ASDL235DAFVL34"
  // store.orientation = "L"
  // store.sensorTestResult = 2;
  // store.overallResult = 2;
  // store.instruction = 'anything';
  // countingBLE.value = true;

  // const bootResultTestArray = [1400, 2100, 3500, 5600, 7000, 1000, 2200, 3550, 5140, 6970];

  // bootResultComponentRef.value.setResults(bootResultTestArray);
...
function processData(decodedOscMessage){
  ...
  if(commandType === "DATA"){
    ...
    if(commandID === BackEndCommand.BootMeasurementData || commandID === BackEndCommand.BootRecordBoot){
      console.log("Measurement Data");
      chartComponentRef.value.processMessage(decodedOscMessage);
    }
   // if DATA + FinalResults => TestingComponent
    else if(commandID === BackEndCommand.BootFinalResults){
    ...
    else if(commandID === BackEndCommand.BootFinalStatusResults){
    ...
    else if(commandID === BackEndCommand.BootSizeOrientationRFID){
  ...
  // CMD
    ...
    if(commandID === BackEndCommand.BootInitTesting){
      console.log("InitTesting");
      
      // chartComponentRef.value.processMessage(decodedOscMessage);
    }
    // if CMD + ReadyToTest = Operative => Read RFID
    else if(commandID === BackEndCommand.BootReadRFID){
    ...
    else if(commandID === BackEndCommand.BootResetting){
  ...
  // ACK
    ...
    if(commandID === BackEndCommand.BootReadRFID){
      console.log("rfid ok");
    }
    // Lightbarrier (OFF)
    else if(commandID === FrontEndCommand.LightBarrier){
  ...
  // ACK/NACK
    ...
    /*
    if(commandID === BackEndCommand.BootReadRFID && commandType == "NACK"){
      console.log("rfid not ok")
      store.errorActive = true;
      store.errorID = 0;
      store.errorMessage = "No RFID!"
    }
    else if(commandID === BackEndCommand.BootReadRFID && commandType == "ACK"){
      console.log("rfid ok");
    }
